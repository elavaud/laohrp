 ...
 *}
{strip}
{assign var="pageTitle" value="user.registerA"}
...
{if !$implicitAuth}
	<tr valign="top">
		<td title="ຊື່ຜູ້ໃຊ້" width="20%" class="label">[?] {fieldLabel name="username" required="true" key="user.username"}</td>
	...
	<tr valign="top">
		<td></td>
		<td title="ຊື່ຜູ້ໃຊ້ຕ້ອງພິມຕົວອັກສອນນ້ອຍ, ຕົວເລກ ແລະ ເຄື່ອງຫມາຍຂິດເທົ່ານັ້ນ" class="instruct">[?] {translate key="user.register.usernameRestriction"}</td>
	</tr>
	{/if}{* !$existingUser *}

	<tr valign="top">
		<td title="ລະຫັດຜ່ານ" class="label">[?] {fieldLabel name="password" required="true" key="user.password"}</td>
	...
	{if !$existingUser}
		<tr valign="top">
			<td></td>
			<td title="ລະຫັດຜ່ານຢ່າງຫນ້ອຍຕ້ອງມີ 6 ຕົວອັກສອນ" class="instruct">[?] {translate key="user.register.passwordLengthRestriction" length=$minPasswordLength}</td>
		</tr>
		<tr valign="top">
			<td title="ພິມລະຫັດຜ່ານອີກຄັ້ງ" class="label">[?] {fieldLabel name="password2" required="true" key="user.register.repeatPassword"}</td>
		...
		<tr valign="top">
			<td title="ຊື່ຫຍໍ້" class="label">[?] {fieldLabel name="initials" key="user.initials"}</td>
		...
		<tr valign="top">
			<td title="ເພດ" class="label">[?] {fieldLabel name="gender-m" key="user.gender"}</td>
			<td class="value">
...
-->
		...
		<tr valign="top">
			<td title="ຢືນຢັນ ອິເມວ" class="label">[?] {fieldLabel name="confirmEmail" required="true" key="user.confirmEmail"}</td>
...
-->
		<tr valign="top">
			<td title="ເບີໂທລະສັບ" class="label">[?] {fieldLabel name="phone" key="user.phone"}</td>
			<td class="value"><input type="text" name="phone" id="phone" value="{$phone|escape}" size="15" maxlength="24" class="textField" /></td>
		</tr>

		<tr valign="top">
			<td title="ແຟກ" class="label">[?] {fieldLabel name="fax" key="user.fax"}</td>
			<td class="value"><input type="text" name="fax" id="fax" value="{$fax|escape}" size="15" maxlength="24" class="textField" /></td>
		</tr>

		<tr valign="top">
			<td title="ທີ່ຢູ່ທາງໄປສະນີ" class="label">[?] {fieldLabel name="mailingAddress" key="common.mailingAddress"}</td>
			<td class="value"><textarea name="mailingAddress" id="mailingAddress" rows="3" cols="40" class="textArea">{$mailingAddress|escape}</textarea></td>
		</tr>

		<tr valign="top">
			<td title="ປະເທດ" class="label">[?] Country</td>
			<td class="value">
		...
		<tr valign="top">
			<td title="ສົ່ງອີເມວໃຫ້ຂ້ອຍລວມທັງຊື່ຜູ້ໃຊ້ແລະລະຫັດຜ່ານ" class="label">[?] {fieldLabel name="sendPassword" key="user.sendPassword"}</td>
			<td class="value">
...
<br />
<p><input type="submit" value="{translate key="user.registerA"}" class="button defaultButton" /> <input type="button" value="{translate key="common.cancel"}" class="button" onclick="document.location.href='{url page="index" escape=false}'" /></p>
