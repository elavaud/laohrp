 ...
 *}
{strip}
{assign var="pageTitle" value="user.userHomeA"}
{assign var="pageCrumbTitle" value="user.userHome"}
...
{foreach from=$userJournals item=journal}
	...
	<table width="100%" class="info">
		...
		{if $isValid.SectionEditor.$journalId}
			...
			<tr>
				<td>&#187; <a href="{url journal=$journalPath page="sectionEditor"}">{$user->getSecretaryEthicsCommittee()} {*{translate key="user.role.sectionEditor"}*}ເລຂາ</a></td>
				<td></td>
				<td>{if $sectionEditorSubmissionsCount[0]}
						<a href="{url journal=$journalPath page="sectionEditor" op="index" path="submissionsInReview"}">{*{translate key="common.queue.short.submissionsInReview"}*}ໃນການທົບທວນ ({$sectionEditorSubmissionsCount[0]})</a>
					{else}<span class="disabled">{*{translate key="common.queue.short.submissionsInReview"}*}ໃນການທົບທວນ (0 )</span>{/if}
				</td>
				<td align="left"><a href="{url journal=$journalPath page="sectionEditor" op="index" path="submissionsArchives"}">{*translate key="common.queue.short.completed"*}ບ່ອນ​​ເກັບ​ຮັກສາເອກະສານ</a></td>
				<td align="right"></td>
			</tr>
		{/if}
		...
		{if $isValid.Author.$journalId}
			...
			<tr>
				<td>&#187; <a href="{url journal=$journalPath page="author"}">{translate key="user.role.author"} / ຜູ້ເຮັດການຄົ້ນຄວ້າ</a></td>
				<td colspan="2" align="center">
					{if $authorSubmissionsCount[1]}
						<a href="{url journal=$journalPath page="author" path="completed"}">{$authorSubmissionsCount[1]} {translate key="common.queue.short.completed"} / ບ່ອນ​​ເກັບ​ຮັກສາເອກະສານ</a> 
					{else}
						<span class="disabled">{translate key="common.queue.short.completed"} / ບ່ອນ​​ເກັບ​ຮັກສາເອກະສານ (0)</span>
					{/if}				
				</td>				
				<td align="left">{if $authorSubmissionsCount[0]}
						<a href="{url journal=$journalPath page="author"}">{$authorSubmissionsCount[0]} {translate key="common.queue.short.active"} / ປະຕິບັດ</a>
					{else}<span class="disabled">0 {translate key="common.queue.short.active"} / ປະຕິບັດ</span>{/if}
				</td>
			...
			<tr>
				...
				<td align="right">
					{if $reviewerSubmissionsCount[0]}
						<a href="{url journal=$journalPath page="reviewer"}">{translate key="common.queue.short.active"} / ປະຕິບັດ ({$reviewerSubmissionsCount[0]})</a>
					{else}
						<span class="disabled">{translate key="common.queue.short.active"} / ປະຕິບັດ (0)</span>
					{/if}
				</td>
				<td align="center">
					{if $reviewerSubmissionsCount[1]}
					<a href="{url journal=$journalPath page="reviewer" path="completed"}">{translate key="common.queue.short.completed"} / ບ່ອນ​​ເກັບ​ຮັກສາເອກະສານ ({$reviewerSubmissionsCount[1]})</a>
					{else}
	...
	{if $currentJournal}
		...
		<ul class="plain">
			...
			<li>
				&#187;
				{if $allowRegReviewer}
					{url|assign:"userHomeUrl" page="user" op="laohrp"}
