...
{if $currentJournal->getSetting('submissionFeeEnabled')}
	...
	{if $submissionPayment}
		{if ($submissionPayment->getPayMethodPluginName()) == 'ManualPayment'}<b>Payment received</b> in cash or cheque<br/>Date: {$submissionPayment->getTimestamp()|date_format:$datetimeFormatLong}
		{elseif ($submissionPayment->getPayMethodPluginName()) == 'Waiver'}<b>Payment waived</b> by the secretary<br/>Date: {$submissionPayment->getTimestamp()|date_format:$datetimeFormatLong}
		{/if}
	{elseif $submission->getLocalizedStudentInitiatedResearch() == 'Yes'}
		<b>Exempt of fee </b>(student research)
	{else}
