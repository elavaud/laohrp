...
<tr valign="top">
	...
	{if $proposalStatus == PROPOSAL_STATUS_SUBMITTED || $proposalStatus == PROPOSAL_STATUS_RESUBMITTED }
		<td title="Please check all the information submitted and the files downloaded (SUMMARY link above) in order to decide if the proposal is incomplete or not." class="label" width="20%">{*{translate key="editor.article.selectInitialReview"}*}[?] ເລືອກການທົບທວນລິ້ເລີ້ມ</td>
		<td width="80%" class="value">
			<select id="decision" name="decision" size="1" class="selectMenu">
				{html_options_translate options=$initialReviewOptions selected=1}
			</select>
			<input type="submit" onclick="return confirm('{translate|escape:"jsparam" key="editor.submissionReview.confirmInitialReview"}')" name="submit" value="{translate key="editor.article.recordS"}"  class="button" />			
		</td>

	{elseif $proposalStatus == PROPOSAL_STATUS_CHECKED}
		<td title="Please select the type of review needed. If the proposal does'nt need any review, please select 'Exemption of Review'." class="label" width="20%">{*{translate key="editor.article.selectExemptionDecision"}*}[?] ເລືອກຂະບວນການທົບທວນ</td>
		<td width="80%" class="value">
			<select id="decision" name="decision" size="1" class="selectMenu">
				{html_options_translate options=$exemptionOptions selected=1}
			</select>
			<input type="submit" id="notFullReview" onclick="return confirm('{translate|escape:"jsparam" key="editor.submissionReview.confirmExemption"}')" name="submit" value="{translate key="editor.article.recordS"}"  class="button" />
		</td>
	{elseif $proposalStatus == PROPOSAL_STATUS_REVIEWED && $submission->isDueForReview()==1}
		...
		<td width="80%" class="value">
				...
				<input type="submit" onclick="return confirm('{translate|escape:"jsparam" key="editor.submissionReview.confirmReviewSelection"}')" name="submit" value="{translate key="editor.article.recordS"}"  class="button" />
		</td>
	{elseif ($articleMoreRecent && $proposalStatus == PROPOSAL_STATUS_REVIEWED && $lastDecisionArray.decision == SUBMISSION_EDITOR_DECISION_RESUBMIT)}
		...
		<td width="80%" class="value">
				...
				<input type="submit" onclick="return confirm('{translate|escape:"jsparam" key="editor.submissionReview.confirmDecision"}')" name="submit" value="{translate key="editor.article.recordDecisionS"}"  class="button" />
		</td>	
	{/if}
	</tr>
...
{/if}
	<tr id="approvalDateRow">
		<td title="Please select the date of approbation. If no date is entered, today's date will be selected." class="label">{*{translate key="editor.article.setApprovalDate"}*}[?] ລົງວັນທີອານຸມັດ</td>
		<td class="value">
...
{if $proposalStatus == PROPOSAL_STATUS_EXPEDITED || $proposalStatus == PROPOSAL_STATUS_ASSIGNED}	
	<tr>
		<td title="Please select and upload the approbation letter. This optionnal step can always be realized after." class="label" width="20%">{*{translate key="editor.article.uploadFinalDecisionFile"}*}[?] ອັບໂຫລດຟາຍເອກະສານຕັດສິນ</td>
		<td width="80%" class="value">
...
<tr valign="top">
	<td title="Final decision decided." class="label">{*{translate key="editor.article.finalDecision"}*}[?] ການຕັດສິນໃຈລ້າສຸດ</td>
	<td class="value">
...
{if ($proposalStatus == PROPOSAL_STATUS_RETURNED) || ($proposalStatus == PROPOSAL_STATUS_RESUBMITTED) || ($proposalStatus == PROPOSAL_STATUS_REVIEWED && $lastDecisionArray.decision == SUBMISSION_EDITOR_DECISION_RESUBMIT) }
	...
	{if !$articleMoreRecent}
		<td title="Current status of the proposal." class="label" width="20%">[?] {*{translate key="editor.article.submissionStatus"}*}ສະຖານະ</td>
...
{if $proposalStatus == PROPOSAL_STATUS_EXEMPTED}
	...
	<form method="post" action="{url op="recordReasonsForExemption"}">
		...
		<tr valign="top">
			<td title="If not already done, please select the resasons of the exemption." class="label">{*{translate key="editor.article.reasonsForExemption"}*}[?] ເຫດຜົນຂອງການຍົກເວັ້ນ</td>
...
<form method="post" action="{url op="uploadDecisionFile" path=$submission->getId()}"  enctype="multipart/form-data">
	<tr valign="top">
		<td title="Please upload the decision letter here." class="label">{*Upload Decision File*}[?] ອັບໂຫລດຟາຍເອກະສານຕັດສິນ</td>
		<td class="value">		
			<input type="file" class="uploadField" name="finalDecisionFile" id="finalDecisionFile"/>
			<input type="submit" class="button" value="{translate key="common.uploadS"}" />
		</td>
	</tr>
</form>
{/if}
...
<tr valign="top">
	<td title="Please click on the mail icon for sending an email to investigator. Please click on the speech bubble icon for opening the chat room between you and the investigator." class="label">{*{translate key="submission.notifyAuthor"}*}[?] ແຈ້ງຕໍ່ຜູ້ເຮັດການຄົ້ນຄວ້າ</td>
	<td class="value">
		...
		{if $submission->getMostRecentEditorDecisionComment()}
			...
			&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:openComments('{url op="viewEditorDecisionComments" path=$submission->getId() anchor=$comment->getId()}');" class="icon">{icon name="comment"}</a>&nbsp;&nbsp;Last "chat" message: {$comment->getDatePosted()|date_format:$dateFormatLong}
		{else}
