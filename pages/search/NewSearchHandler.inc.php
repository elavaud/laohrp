...
class NewSearchHandler extends Handler {
	...
	function advanced() {
        ...
        $proposalCountries =& $countryDAO->getProvincesOfLaos();
	...
	function advancedResults() {
		...
		$templateMgr->assign('country', $countryDAO->getProvinceOfLaos($country));
		...
		foreach ($results as $result) {
			foreach ($columns as $index => $junk) {
				if ($index == 'investigator') {
					...
					$columns[$index] = date("d M Y", strtotime($result->getStartDate(Locale::getLocale())))." to ".date("d M Y", strtotime($result->getEndDate(Locale::getLocale())));
				} elseif ($index == 'area') {
					...
					if ($result->getLocalizedStudentInstitution() != "NA") $columns[$index] = $result->getLocalizedStudentInstitution(); 
					else $columns[$index] = "Non Student Research";
				} elseif ($index == 'academic_degree') {
					if ($result->getLocalizedAcademicDegree() != "NA") $columns[$index] = $result->getLocalizedAcademicDegree();
					else $columns[$index] = "Non Student Research";
				}
	...
	function setupTemplate($subclass = false, $articleId = null) {
		...
		if ($articleId == null) {$templateMgr->assign('pageHierarchy',
			...
			$subclass ? array(array(Request::url(null, 'search'), 'navigation.search'), array(Request::url('laohrp', 'search','advancedResults'), 'search.searchResults'))
				: array()
			);
		}
