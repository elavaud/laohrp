	...
	*/
	...
	function generateSubmissionsReport($args) {
		...
		if ($multiCountry) {
			...
			else array_push($criterias, ('is conducted only in Cambodia'));
		} 
		if ($nationwide) {
		...
		if (!empty($countryField)){
			$provincesOfLaosDAO =& DAORegistry::getDAO('ProvincesOfLaosDAO');
			$countryCriteria = "";
			$present = false;
			foreach ($countryField as $country){
				if(!empty($country)){
					$present = true;
					if ($countryCriteria == "" || $countryCriteria == null) $countryCriteria = $provincesOfLaosDAO->getProvinceOfLaos($country).' ';
					else $countryCriteria .= 'or '.$provincesOfLaosDAO->getProvinceOfLaos($country).' ';
				}
			}
			if ($present == true) array_push($criterias, ("the list of provinces includes ".$countryCriteria));
		}	
